...
} from '@/components/candidate-profile-modal';
...
export default function EmployerJobDetailPage() {
  ...
  const filteredCandidates = useMemo(() => {
    return sampleCandidates.filter((candidate) => {
      ...
      if (
        experience &&
        experience !== 'All' &&
        !candidate.experienceRange.startsWith(experience.split(' ')[0])
      )
        return false;
      if (
        education.length &&
        !education.includes(candidate.education) &&
  ...
  // This is container for each candidate
  const CandidateCard = ({ candidate }: { candidate: any }) => {
    return (
      <div className='group flex flex-col gap-4 rounded-3xl border border-border hover:border-sky-700 bg-background p-4 sm:flex-row sm:items-center sm:justify-between transition-all'>
        <div className='flex flex-1 items-center gap-4'>
          <div className='flex h-16 w-16 shrink-0 items-center justify-center rounded-3xl bg-muted text-muted-foreground'>
            <span className='text-base font-semibold uppercase text-slate-700'>
              {candidate.name
                .split(' ')
                .map((n: string) => n[0])
                .join('')}
        ...
        <div className='flex items-center gap-2'>
          <Button
            variant='outline'
            size='sm'
            onClick={() => setSelectedCandidate(candidateProfileData)}
            className='bg-sky-100 text-sky-600 group-hover:bg-sky-700 hover:bg-sky-700 group-hover:text-white hover:text-white px-6 h-10 font-semibold gap-2 transition-all'
          >
            View Profile
  ...
  return (
    <div className='min-h-screen bg-background text-foreground'>
      <div className='flex rounded-[0.5rem] justify-left align-center font-semibold bg-muted w-full px-10 py-5 text-lg text-black'>
        Job Details
      </div>
          ...
          <section className='space-y-6 rounded-[2rem] border border-border bg-card p-6 shadow-sm lg:p-8'>
            <div className='flex flex-col gap-6 lg:flex-row lg:gap-6'>
              ...
              <div className='flex flex-col items-end gap-4 lg:justify-start'>
                <Button className='gap-2 px-6 py-3 w-60 h-10 bg-sky-700 text-base'>
                  Apply Now
                  <ArrowRight className='size-4' />
                </Button>
          ...
          <section className='mt-8 rounded-[2rem] border border-border bg-card p-6 shadow-sm lg:p-8'>
            <div className='space-y-4'>
              <div className='flex items-center justify-between gap-4'>
                ...
                <div className='inline-flex items-center w-60 gap-2 rounded-2xl px-3 py-2 justify-center text-sm text-muted-foreground'>
