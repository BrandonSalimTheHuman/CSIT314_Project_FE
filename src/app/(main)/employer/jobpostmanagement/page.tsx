...
const JobCard = ({ job }: { job: (typeof jobPostings)[number] }) => (
  <div className='rounded-2xl border border-border p-6 shadow-sm transition hover:border-sky-600 hover:shadow-md bg-white'>
    <div className='flex items-center gap-4'>
      ...
      <div className='flex flex-col gap-1'>
        <div className='text-lg font-medium text-foreground'>{job.company}</div>
...
export default function EmployerJobPostManagementPage() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobPostings.filter((job) => {
      ...
      return [job.company, job.position, job.location, job.jobType]
        .join(' ')
        .toLowerCase()
        .includes(query);
    });
  ...
  return (
    <div className='min-h-screen bg-white text-foreground'>
      <div className='w-full bg-white'>
        <div className='flex flex-col gap-6 bg-muted w-full px-10 py-5'>
          <h1 className='text-lg font-semibold text-black'>
            Job Posting Management
          </h1>

          <div className='flex items-center gap-4'>
            <div className='flex flex-1 items-center gap-2 rounded-lg bg-white p-2 shadow-sm border border-slate-100'>
              <div className='relative flex-1'>
                <Search className='pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-blue-500' />
                <Input
                  className='border-none bg-transparent pl-12 text-base focus-visible:ring-0 placeholder:text-slate-400'
                  placeholder='Job title, Keyword...'
                  value={search}
                  onChange={(e) => setSearch(e.target.value)}
                />
              </div>
              <Button className='bg-[#0066cc] px-8 py-6 text-base font-medium hover:bg-[#0052a3]'>
                Find Job
              </Button>
            </div>
            ...
            <Button
              variant='outline'
              className='px-12 gap-2 border-sky-600 bg-[#f1f2f4] text-[#0066cc] text-base font-semibold hover:bg-slate-100'
            >
              Filter
            </Button>
          </div>
      ...
      <main className='mx-auto max-w-[1400px] px-4 py-8 sm:px-6 lg:px-8'>
        <div className='mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between'>
          ...
          <div className='grid grid-cols-2 items-center gap-3'>
            <Select value={sort} onValueChange={setSort}>
              <SelectTrigger className='w-[180px] h-12 bg-white border-slate-200 text-slate-600 py-5'>
        ...
        <div className='grid gap-6 lg:grid-cols-3'>
          {currentJobs.map((job) => (
            <JobCard key={`${job.company}-${job.position}`} job={job} />
          ))}
        </div>
